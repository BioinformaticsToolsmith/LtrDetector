    ...
    for r in repeats:
        ...
        while i < len(copies):
            ...
            if tupleCompare(prev, curr):
                info = (prev[0], int(prev[1]), int(curr[2]), int(
                    prev[1]), int(prev[2]), int(curr[1]), int(curr[2]))
                next.append(info)
...
def tupleCompare(first, second):
    ...
    return distance > 200 and distance < 12000
...
def buildGroundTruth(repeats, repeatMaskerFile, chromFile,queryFile, name):
...
def processChromosome(directory, repeatsFile):
    ...
    for p in pairs:
        ...
        fa_file = p[0]
        out_file = p[1]
        name = directory+"/"+ os.path.splitext(fa_file)[0].split('/')[2]
        print "building "+name
        buildGroundTruth(repeats, out_file, fa_file,combined, name)      
